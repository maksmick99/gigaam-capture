...
- **Воспроизводимость ASR:** runtime `gigaam` зафиксирован коммитом
  `7447938d791c4f3e643386ee22c33777004293a5` в extra `asr`; добавлен smoke-тест
  `tests/test_model_smoke.py` (`@pytest.mark.model`) для загрузки `v3_e2e_rnnt`
  и транскрибации короткого WAV — не входит в обычный `pytest`.

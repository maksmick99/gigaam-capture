...
> Контекст: приложение находится в `apps/gigaam-capture` и намеренно отделено от основной библиотеки `gigaam`, чтобы в будущем его можно было вынести в отдельный репозиторий.
